...
export const ICONS = {
  ...
  accessible_completion: `<svg viewBox="0 0 48 48" width="48" height="48" fill="none"><circle cx="24" cy="24" r="22" stroke="currentColor" stroke-width="1.5"/><path d="M14 24.5l6.2 6.2L34 16.5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
...
export function copyForResult(result) {
  const secs = ((result.features.totalMs || 0) / 1000).toFixed(1);
  const f = result.features;
  const evidence = buildEvidence(f, result.state);

  switch (result.state) {
    case 'accessible_completion':
      return {
        title: 'Human enough.',
        note: 'Completed through the accessible keyboard path.',
        evidence: 'Accessible keyboard completion · pointer trajectory classifier not applied',
        timeLabel: `Observed in ${secs} seconds`,
        caveat: 'Keyboard bypasses pointer biometrics. No fake human-likelihood score is assigned.',
        insight: 'Accessible keyboard completion. Pointer trajectory classifier not applied.'
      };
    case 'human_like':
      return {
        title: 'Human enough.',
        note: 'The movement showed structured variation across timing, speed and path shape.',
        evidence,
        timeLabel: `Observed in ${secs} seconds`,
        caveat: 'One interaction signal — not proof of identity. Heuristic scores are not calibrated probabilities.',
...
function buildEvidence(f, state) {
  if (state === 'insufficient_signal') {
    return `${f.sampleCount} samples · ${((f.movementTime || 0) / 1000).toFixed(2)}s · ${Math.round(f.pathLength || 0)}px path`;
  }
  if (state === 'accessible_completion') {
    return 'Keyboard path · biometric scores null';
  }
  const parts = [
    `${Math.round((f.pathEfficiency || 0) * 100)}% efficiency`,
    `vCV ${numStr(f.velocityCV)}`,
